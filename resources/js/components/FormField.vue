  ...
  <div class="permissions-w-full md:permissions-py-3 permissions-px-7 text-sm">
    <div class="permissions-w-full permissions-mb-4 space-x-2">
            <span
                ...
                @click="checkAll()">{{ __('Select all') }}</span>
      <span
          class="shadow relative bg-primary-500 hover:bg-primary-400 text-white dark:text-gray-900 cursor-pointer rounded text-sm font-bold focus:outline-none focus:ring ring-primary-200 dark:ring-gray-600 inline-flex items-center justify-center h-9 px-3 shadow relative bg-primary-500 hover:bg-primary-400 text-white dark:text-gray-900"
          @click="uncheckAll()">{{ __('Do not select any') }}</span>
    </div>
    <div
        v-if="field.withGroups"
        class="permissions-w-full permissions-grid 2xl:permissions-grid-cols-5 xl:permissions-grid-cols-4 lg:permissions-grid-cols-3 permissions-grid-cols-2 md:permissions-py-3 permissions-px-1">
    ...
    <div
      ...
      <div v-for="(label, option) in field.options" :key="option" class="flex mb-2">
        ...
        />
        <label :for="field.name" class="permissions-pl-2 permissions-w-full permissions-capitalize" @click="toggleOption(option)"
               v-text="label"></label>
      </div>
    </div>
